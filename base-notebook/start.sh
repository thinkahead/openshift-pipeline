...
# the madi user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the madi user to the desired the user (NB_USER)
    if id madi &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" madi 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the madi user:"
            _log "- username: madi       -> ${NB_USER}"
            _log "- home dir: /home/madi -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the madi user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the madi home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "madi" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/madi to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/madi/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/madi to /home/${NB_USER}!"
                _log "Attempting to symlink /home/madi to /home/${NB_USER}..."
                if ln -s /home/madi "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/madi to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/madi/"* ]]; then
...
# user we started as.
    ...
    JOVYAN_UID="$(id -u madi 2>/dev/null)"  # The default UID for the madi user
    JOVYAN_GID="$(id -g madi 2>/dev/null)"  # The default GID for the madi user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old madi user to nayvoj ($(id -u madi):$(id -g madi))"
            ...
            sed --expression="s/^madi:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/madi:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "madi" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/madi. You must run as root to rename the home directory!"
            fi
        else
    ...
    # are not root.
    if [[ "${NB_USER}" != "madi" && "${NB_USER}" != "$(id -un)" ]]; then
    ...
    if [[ ! -w /home/madi ]]; then
        _log "WARNING: no write access to /home/madi. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
